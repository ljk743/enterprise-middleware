...
/**
 * <p>This class produces a RESTful service exposing the functionality of {@link TravelAgentBookingService}.</p>
 *
 ...
 * @see TravelAgentBookingService
 * @see Response
 */
@Path("/agentbookings")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class TravelBookingRestService {
    @Inject
    @Named("logger")
    Logger log;

    @Inject
    TravelAgentBookingService service;
    ...
    public Response retrieveAllBookings(@QueryParam("flightid") Long flightId, @QueryParam("orderdate") String orderDate) {
        ...
        if (flightId == null && orderDate == null) {
            travelAgentBookings = service.findAllOrdered();
        } else if (orderDate == null || flightId == null) {
            throw new RestServiceException("Bad Request", Response.Status.BAD_REQUEST);
        } else {
            travelAgentBookings = service.findAllByFlightId(flightId);
            travelAgentBookings.retainAll(service.findAllByOrderDate(orderDate));
        }
    ...
    /**
     * <p>Search for and return a Booking identified by customer id address.<p/>
     *
    ...
    public Response retrieveBookingsByCustomerId(
            @Parameter(description = "Customer id of Booking to be fetched", required = true)
            @PathParam("customerid")
            Long customerId) {

        TravelAgentBooking travelAgentBooking;
        try {
            travelAgentBooking = service.findByCustomerId(customerId);
        } catch (NoResultException e) {
            // Verify that the booking exists. Return 404, if not present.
            throw new RestServiceException("No Booking with the customer id " + customerId.toString() + " was found!", Response.Status.NOT_FOUND);
        }
        return Response.ok(travelAgentBooking).build();
    }
    ...
    public Response retrieveBookingById(
        ...
        TravelAgentBooking travelAgentBooking = service.findById(id);
        if (travelAgentBooking == null) {
            // Verify that the travelAgentBooking exists. Return 404, if not present.
            throw new RestServiceException("No Booking with the id " + id + " was found!", Response.Status.NOT_FOUND);
        }
        log.info("findById " + id + ": found Booking = " + travelAgentBooking);
     ...
     *
     * @param travelAgentBooking The Booking object, constructed automatically from JSON input, to be <i>created</i> via
     *                           {@link TravelAgentBookingService#create(TravelAgentBooking)}
    ...
    public Response createBooking(
        ...
        if (customerService.findById(travelAgentBooking.getCustomerId()) == null) {
            throw new RestServiceException("Bad Request, customer not found !", Response.Status.BAD_REQUEST);
        }
        if (flightService.findById(travelAgentBooking.getFlightId()) == null) {
        ...
        log.info("createBooking completed. Booking = " + travelAgentBooking);
     ...
     * @param travelAgentBooking The Booking object, constructed automatically from JSON input, to be <i>updated</i> via
     *                           {@link TravelAgentBookingService#update(TravelAgentBooking)}
     * @return A Response indicating the outcome of the create operation
     */
    @PUT
    @Path("/{id:[0-9]+}")
    ...
    public Response updateBooking(
        ...
        if (travelAgentBooking == null || travelAgentBooking.getId() == null) {
            throw new RestServiceException("Invalid Booking supplied in request body", Response.Status.BAD_REQUEST);
        }

        if (travelAgentBooking.getId() != null && travelAgentBooking.getId() != id) {
        ...
        if (service.findById(travelAgentBooking.getId()) == null) {
            // Verify that the booking exists. Return 404, if not present.
            throw new RestServiceException("No Booking with the id " + id + " was found!", Response.Status.NOT_FOUND);
        }
        ...
        try {
            ...
            service.update(travelAgentBooking);
        ...
        log.info("updateBooking completed. Booking = " + travelAgentBooking);
        return builder.build();
    }
    ...
    public Response deleteBooking(
        ...
        try {
            service.delete(travelAgentBooking);
        ...
        log.info("deleteBooking completed. Booking = " + travelAgentBooking);
        return builder.build();
    }
}